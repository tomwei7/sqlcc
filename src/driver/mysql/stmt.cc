...
static void bind_to_value(MYSQL_FIELD *field, MYSQL_BIND *bind, Value &value) {
  ...
  case (MYSQL_TYPE_LONGLONG):
    value = *(int64_t *)(bind->buffer);
    break;
  case (MYSQL_TYPE_DOUBLE):
    value = *(double *)(bind->buffer);
    break;
  case (MYSQL_TYPE_STRING):
    value = std::string((char *)bind->buffer, *(bind->length));
    break;
  default:
    throw std::runtime_error("unsupport bind type");
  }
}
...
Statement::Statement(Connection *conn, const std::string &query)
    : conn_(conn), query_(query), stmt_(nullptr), bind_(nullptr),
      bind_size_(0) {
